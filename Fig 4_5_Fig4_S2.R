...
setwd(
  "D:/Prateek/TIFR/MS Lab/Ecad Gradient/locfit analysis/R files version
control/Ecad_project/"
)
...
control/Ecad_project/"
filename <-
  "Autonomous peri Lgl 05052019 14122018_outliers removed"
format <- ".csv"
...
rawheight <- function(df) {
  ...
  for (i in seq_along(df$Mean)) {
    height[i] <- (i - 1) * 0.28 #the height of the z slice
    265}
  height
}
...
dev.off ()
#to plot lineplots for cell height
s <-
  ggplot(
    data = graph.lines,
    aes(
